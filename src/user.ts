...
export class Deck {
  constructor(deck_action: CreateDeck, player_id: number) {
    ...
    this.card_back =
      'cardback_starter_' + deck_action.main_faction.toLowerCase();
  ...
  getCards(start_id: number, is_left: boolean) {
    const cards_code = this.deck_code
      .replace(/~[^|]*/g, '')
      .split('|')[1]
      .split(';');
...
export class User {
  constructor(user_name: string) {
    ...
    this.banned = false;
    this.packs = [];
    this.levels = { USA: 8, Japan: 8, Germany: 8, Soviet: 8, Britain: 8 };
    this.xp = { USA: 0, Japan: 0, Germany: 0, Soviet: 0, Britain: 0 };
  }
  async store() {
